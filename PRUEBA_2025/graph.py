...
def plot_principal_fields(nodes, elements, u_global, title_prefix, sigma_y_tension, sigma_y_compression):
        ...
        # Función para imprimir los valores máximos y mínimos de un campo
        def print_min_max(field, name):
            max_value = max(field.values())
            min_value = min(field.values())
            print(f"{name} - Max: {max_value:.2f} MPa, Min: {min_value:.2f} MPa")

        # Imprimir los valores máximos y mínimos de cada campo
        print_min_max(sigma1, r"$\sigma_1$")
        print_min_max(sigma2, r"$\sigma_2$")
        print_min_max(eps1, r"$\varepsilon_1$")
        print_min_max(eps2, r"$\varepsilon_2$")
