...
def main (title, lc=10, self_weight=False, distribuited_force_x = False, distribuited_force_y = False, Topologic_Optimization = False, def_scale=1, force_scale=1e-2, reaction_scale=1e-2):
    ...
    input_file = "PRUEBA_2025/geo.geo"#"VIGA_DOBLE_T/doble_t.geo"
    output_file ="PRUEBA_2025/msh.msh"# "VIGA_DOBLE_T/doble_t.msh"

    rho =  2500e-9#densidad, Kg/mm3 OJOOOOOO
    sigma_y_compression = 40 #MPa
    sigma_y_tension = 0.35*(sigma_y_compression**0.5)#Mpa
    E = 4700*(sigma_y_compression**0.5)#Mpa
    ...
    thickness = {'1':1000}#{'1': 115, '2': 120, '3': 120, '4': 120}, tiene que ser con numeros de Plane Surface
    ...
    if distribuited_force_x:
        ...
        #grupo_nodos = grupos['Fuerza 1']
        #fuerza_total_x = 1200000 #N NO N/m
        #apply_distributed_force_x(grupo_nodos, fuerza_total_x, estructure)
        #grupo_nodos = grupos['Fuerza 2']
        #fuerza_total_x = 12000000 #N NO N/m
        #apply_distributed_force_x(grupo_nodos, fuerza_total_x, estructure)
        pass

    if distribuited_force_y:
        # Aplicar fuerza distribuida a los nodos
        fuerza_total = 65000 #N
        grupo_nodos = grupos['Fuerza 1']
        fuerza_total_y = fuerza_total*(60/279) #N NO N/m
        apply_distributed_force_y(grupo_nodos, fuerza_total_y, estructure)
        grupo_nodos = grupos['Fuerza 2']
        fuerza_total_y = fuerza_total*(219/279) #N NO N/m
        apply_distributed_force_y(grupo_nodos, fuerza_total_y, estructure)
        pass
    ...
    #Calculo de ditnaica ntre nodos
    #for element in elements:
    #    print(element.node_list)

    #    print('La distancia entre los dos primeros nodos es')
    #    distnaica = 0
    #    vector_1 = [element.node_list[0].x,element.node_list[0].y]
    #    vector_2 = [element.node_list[1].x,element.node_list[1].y]

    #    dist = np.linalg.norm(np.array(vector_1) - np.array(vector_2))
    #    print(f"{dist/10} cm")
...
if __name__ == "__main__":
    title = 'Results' #Debe estar gentro de la carpeta GRAFICOS
    lc = 65
    main(title, lc, self_weight=True, distribuited_force_x = False, distribuited_force_y = True, Topologic_Optimization=False, def_scale=1000, force_scale=1e-2, reaction_scale=5e-3)
